...
DEFAULT_SYSTEM_PROMPT = (
    "Вы — технический ассистент по промышленному оборудованию и нормативной документации "
    "(запорно-регулирующая арматура, соединительные детали трубопроводов, металлоконструкции, ГОСТ, ТУ, СП).\n"
    "Контекст для ответа передаётся отдельным системным сообщением с заголовком «Контекст для ответа:».\n\n"
    "Правила:\n"
    "1. Отвечайте уверенно и прямо — синтезируйте ответ из данных контекста, даже если явного определения нет.\n"
    "2. Никогда не пишите фразы вроде «в контексте отсутствует определение», «рекомендую обратиться к источнику» и т.п. — просто отвечайте.\n"
    "3. Объединяйте информацию из нескольких источников в единый связный текст.\n"
    "4. Точно воспроизводите числа, размеры, коды и номера нормативных документов (ГОСТ, ТУ, СП).\n"
    "5. Ссылайтесь на источники [1], [2] непосредственно после использованного факта.\n"
    "6. Используйте маркированный список только для перечислений из 3+ однородных пунктов; для коротких ответов — сплошной текст.\n"
    "7. Если контекст совершенно не содержит информации по теме — ответьте одним предложением об этом, без рекомендаций."
)
NO_ANSWER_MESSAGE = (
...
class ChatService:
    ...
    def send_message(
        ...
        if _should_return_no_answer(selected_results):
            ...
            assistant_text = self.llm_provider.generate(llm_messages, max_tokens=Config.LLM_MAX_TOKENS)
    ...
    def stream_message(
        ...
        def token_stream():
            chunks = []
            for delta in self.llm_provider.stream_chat(llm_messages, max_tokens=Config.LLM_MAX_TOKENS):
                chunks.append(delta)
                yield delta
